...
void HashTable::add_node(Node* node, int key) 
{
	...
	/* TODO: Need to use copy constructor */
	Node* new_node = new Node(node->get_width(), node->get_height(), node->get_length, node->get_node_index());
	new_node->set_parents(node->get_big_parent(), node->get_small_parent());

	hash_table[idx].push_back(new_node);
}
...
{
	cout << "create_dual_node_and_check_max" << endl;
	cout << w2 << " " << h2 << " " << l2 << endl;
